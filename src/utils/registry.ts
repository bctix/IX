...
import { ChatCommand, ChatCommandFlags, ContextCommand, CustomClient } from "../types/bot_types";
...
import { checkChatFlag } from "./utils";
...
async function registerTextCommand(client:CustomClient, cmdModule: ChatCommand) {
    if (checkChatFlag(cmdModule, ChatCommandFlags.Ignore)) {
        return;
    }
...
export async function deployApplicationCommands(client:CustomClient) {
    ...
    try {
        ...
        client.chatcommands.forEach(command => {
            if (checkChatFlag(command, ChatCommandFlags.NoSlash)) return;
